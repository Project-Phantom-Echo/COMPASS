...
def generate_none_empth_modality_list():
    '''
    修改说明：
    恢复 RGB (索引0) 的随机采样，保持四模态随机缺失训练。
    '''
    rgb_ = random.choices([True, False], k=1, weights=[50, 50])

    depth_ = random.choices([True, False], k=1, weights=[60, 40])
    mmwave_ = random.choices([True, False], k=1, weights=[50, 50])
    lidar_ = random.choices([True, False], k=1, weights=[90, 10])

    modality_list = rgb_ + depth_ + mmwave_ + lidar_

    # 兜底检查：万一随机到全 False，就重来
...
def collate_fn_padd(batch):
    all_actions = {'A01': 0, 'A02': 1, 'A03': 2, 'A04': 3, 'A05': 4,
                   ...
                   'A26': 25, 'A27': 26}
    labels = torch.LongTensor([all_actions[t['action']] for t in batch])

    # RGB
    rgb_list = []
    for t in batch:
        r = t.get('input_rgb', None)
        if isinstance(r, np.ndarray) and r.ndim == 3:
            rgb_list.append(r.astype(np.float32, copy=False))
        else:
            rgb_list.append(np.zeros((480, 640, 3), dtype=np.float32))
    rgb_data = torch.from_numpy(np.stack(rgb_list, axis=0)).permute(0, 3, 1, 2)

    # -------- Depth --------
    depth_list = []
    depth_present = []
    for t in batch:
        d = t.get('input_depth', None)
        ok = isinstance(d, np.ndarray)

        if ok:
            if d.shape == (480, 640, 3):       # HWC
                x = d
            elif d.shape == (3, 480, 640):     # CHW -> HWC
                x = np.transpose(d, (1, 2, 0))
            elif d.shape == (480, 640):        # HW -> HWC(3)
                x = np.repeat(d[..., None], 3, axis=2)
            else:
                ok = False

        if not ok:
            x = np.zeros((480, 640, 3), dtype=np.float32)

        depth_list.append(x.astype(np.float32, copy=False))
        depth_present.append(1 if ok else 0)

    depth_data = torch.from_numpy(np.stack(depth_list, axis=0)).permute(0, 3, 1, 2)
    depth_present = torch.tensor(depth_present, dtype=torch.bool)  # [B]
    ...
    mm_present = []
    for t in batch:
        m = t.get('input_mmwave', None)
        ok = isinstance(m, np.ndarray) and m.ndim == 2 and m.shape[1] == 5 and m.shape[0] > 0
        if ok:
            mm = torch.from_numpy(np.array(m, dtype=np.float32, copy=True))
            L = mm.shape[0]
            present = True
        else:
            # 用 1x5 占位，但 length=0 表示“真实为空”
            mm = torch.zeros((1, 5), dtype=torch.float32)
            L = 0
            present = False
        mm_list.append(mm)
        mm_len.append(L)
        mm_present.append(present)

    mmwave_data = pad_sequence(mm_list, batch_first=True, padding_value=0.0)  # [B, Tm, 5]
    mm_len = torch.tensor(mm_len, dtype=torch.long)                           # [B]
    mm_present = torch.tensor(mm_present, dtype=torch.bool)                   # [B]
    # padding_mask: True 表示 padding
    Tm = mmwave_data.shape[1]
    mm_pad_mask = torch.arange(Tm)[None, :].expand(len(batch), Tm) >= mm_len[:, None]  # [B, Tm]
    ...
    li_present = []
    for t in batch:
        l = t.get('input_lidar', None)
        ok = isinstance(l, np.ndarray) and l.ndim == 2 and l.shape[1] == 3 and l.shape[0] > 0
        if ok:
            li = torch.from_numpy(np.array(l, dtype=np.float32, copy=True))
            L = li.shape[0]
            present = True
        else:
            li = torch.zeros((1, 3), dtype=torch.float32)
            L = 0
            present = False
        li_list.append(li)
        li_len.append(L)
        li_present.append(present)

    lidar_data = pad_sequence(li_list, batch_first=True, padding_value=0.0)   # [B, Tl, 3]
    li_len = torch.tensor(li_len, dtype=torch.long)                           # [B]
    li_present = torch.tensor(li_present, dtype=torch.bool)                   # [B]
    Tl = lidar_data.shape[1]
    li_pad_mask = torch.arange(Tl)[None, :].expand(len(batch), Tl) >= li_len[:, None]  # [B, Tl]

    # 你原来的 modality_list 还可以返回，但建议额外带上这些信息
    modality_list = generate_none_empth_modality_list()

    # extra：把长度和 present/mask 都塞进去（训练脚本里你可以打印/统计）
    extra = {
        "depth_present": depth_present,
        "mmwave_present": mm_present,
        "lidar_present": li_present,
        "mmwave_len": mm_len,
        "lidar_len": li_len,
        "mmwave_pad_mask": mm_pad_mask,
        "lidar_pad_mask": li_pad_mask,
    }

    return rgb_data, depth_data, mmwave_data, lidar_data, labels, extra
